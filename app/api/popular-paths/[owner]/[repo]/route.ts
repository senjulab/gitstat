...
export async function GET(
  ...
  try {
    ...
    const popularPathsRes = await installationOctokit.request("GET /repos/{owner}/{repo}/traffic/popular/paths", {
      owner,
      repo,
    });

    return NextResponse.json({
      paths: popularPathsRes.data,
    });

  } catch (error: any) {
    console.error("Popular Paths API Error:", error);
    return NextResponse.json({ error: error.message || "Failed to fetch popular paths data" }, { status: 500 });
  }
}
